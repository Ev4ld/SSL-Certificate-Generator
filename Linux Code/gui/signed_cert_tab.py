...
def create_signed_cert_tab(parent):
    ...
    ttk.Button(scrollable_frame, text="Browse", command=lambda: output_path.set(filedialog.asksaveasfilename(
        title="Save Certificate As",
        filetypes=[
            ("All Cert Formats", "*.crt *.cer *.pem *.pfx *.p7b *.der"),
            ("PEM", "*.pem"), ("CRT", "*.crt"), ("CER", "*.cer"), ("PFX", "*.pfx"), ("P7B", "*.p7b"), ("DER", "*.der")
        ]
    ))).grid(row=row, column=2)
    row += 1
    ...
    def generate():
        try:
            ...
            if ext in [".pem", ".crt"]:
                ...
                with tempfile.NamedTemporaryFile("wb", delete=False, suffix=".pem") as cert_temp, \
                     tempfile.NamedTemporaryFile("wb", delete=False, suffix=".pem") as ca_temp:
